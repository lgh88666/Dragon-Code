...
- **生命周期扩展点**：让用户不用修改 Dragon Code 源码，也能在会话、输入、工具和压缩等关键时机执行自动化动作。
- **同步拦截与异步通知分离**：真正需要改变当前操作的 Hook 同步等待；普通记录、通知和耗时工作可以后台执行。
- **失败隔离**：Hook 配置错误、超时或外部服务失败不能打断 Agent Loop。

### 核心调用链

```text
用户级 / 项目级 hooks.yaml
  → HookLoader 逐条解析、合并和隔离错误
  → HookEngine 按事件与条件筛选
  → HookActionExecutor 执行 Shell / Prompt / HTTP / Subagent
  → HookExecution / HookOutcome
  → AgentEvent 交给 TUI 展示
  → 拦截时恢复输入或产生 hook_denied；非拦截失败继续主流程
```

### 值得记住的设计

1. **Hook 是生命周期插件，不是 Tool**：Tool 由模型主动选择；Hook 由程序在固定事件自动触发。
2. **上下文变量是事件快照**：HookContext 把事件、会话、目录、模式和工具参数等统一装起来，条件和动作读取同一种结构。
3. **拦截必须发生在副作用之前**：`PreToolUse` 先于权限与真实执行；拒绝后磁盘不应变化，并给模型合法的结构化结果。
4. **Prompt 动作不直接改历史**：它生成一次性 `<hook-notification>`，只进入下一次请求，随后清除。
5. **静态命令比字符串拼接安全**：Shell 命令来自配置，动态参数走脱敏 JSON stdin；不把文件路径直接拼进 shell。
6. **异步也要有所有权**：后台 task 由 HookEngine 统一追踪；切换会话或退出时等待有限时间，再取消并清理子进程组。
7. **Subagent 占位是明确边界**：配置格式提前兼容未来能力，但本章不假装已经完成子 Agent 调度。

### 核心源码入口

| 文件 | 阅读重点 |
|---|---|
| `hooks/config.py` | 两层加载、覆盖顺序和逐条错误隔离 |
| `hooks/conditions.py` | 条件如何读取嵌套 HookContext |
| `hooks/actions.py` | 四类动作、超时、脱敏和进程组清理 |
| `hooks/engine.py` | 顺序执行、拦截、only_once 和后台任务所有权 |
| `agent.py` | 输入、工具、压缩、停止和通知事件如何接入 |
| `tui.py` | 会话事件、输入恢复和 Hook 结果如何展示 |

### 测试与证据

- 全量自动化基线：`477 passed, 2 skipped`；最终新增动作与引擎专项测试：`14 passed`。
- tmux：写后 Hook、写前拦截、输入拒绝恢复和退出取消长耗时 Hook 均通过。
- 安全清理：临时配置、脚本、文件、tmux 会话、WSL 虚拟环境和子进程均已清除。
